...
namespace Modules\exchange1c\Services;
...
class Exchange1CService extends Controller
{
    private $categories = [];
    ...
    {
        // Log::info("1c Header:", request()->header());
    ...
    private function catalogFile()
    {
        ...
        if (strpos($filename, 'import_files') !== false) {
            $upload_file = Storage::disk('local')->path("images/{$filename}");
            if (! File::exists(dirname($upload_file))) {
                File::makeDirectory(dirname($upload_file), 0755, true);
            }
        }
    ...
    private function catalogImport()
    {
        ...
        if (strpos($filename, 'import') !== false) {
            $this->parseImport($upload_file);
            echo "success\n";
        } elseif (strpos($filename, 'offers') !== false) {
            $this->parseOffers($upload_file);
            echo "success\n";
        } else {
            echo "failure\n";
            echo $filename;
    ...
    private function parseImport($filename)
    {
        ...
        // dd($results);
        // $categories = Category::query()->get(['id', 'name', 'uuid', 'parent_id']);
        // $categories = $results->pluck(['id', 'name', 'parent_id'], 'uuid');

        // dd($categories);

        // foreach ($results as $result) {
        //     $this->categories[$result->uuid] = $result;
        // }
        ...
        // foreach ($results as $result) {
        //     $this->properties[$result->uuid] = $result;
        // }
        ...
        // foreach ($results as $result) {
        //     $this->values[$result->uuid] = $result;
        // }
        ...
        // foreach ($results as $result) {
        //     $this->filter_groups[$result->uuid] = $result;
        // }
            ...
            foreach ($xml->Товар as $product) {
                ...
                if ($product->ЗначенияСвойств) {
                    ...
                    foreach ($product->ЗначенияСвойств->ЗначенияСвойства as $prop) {
                        ...
                        // key_exists($value, $this->values) and $value = $this->values[$value]['value'];
                        $value == 'true' and $value = 'Есть';

                        $properties[$this->properties[(string) $prop->Ид]->id] = [
                            // 'property' => $this->properties[(string) $prop->Ид],
                            'value' => $value,
                            'position' => $n++,
                        ];
                ...
                $product->categories()->syncWithoutDetaching([$category->id => ['main' => true]]);
                ...
                if (isset($category)) {
                    ...
                    // $product->category->filters()->syncWithoutDetaching($filter_groups);
                }
            }
        }
    }
